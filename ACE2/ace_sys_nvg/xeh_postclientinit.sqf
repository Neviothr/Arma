...
[] spawn {
	private ["_oldVisionMode", "_currentVisionMode","_changedVD","_setVD"];
	_oldVisionMode = 0;
	_changedVD = false;
	while {true} do {
		_currentVisionMode = currentVisionmode player;
		_setVD = player getVariable [QGVAR(vddd),-1];
		...
		_changedVD = (_setVD > 0 && {viewDistance > _setVD}); TRACE_1("CHANGE DETECTED",_changedVD);
		sleep 0.33;
	};
};
