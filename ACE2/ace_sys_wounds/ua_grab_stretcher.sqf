...
// if (((_caller call CBA_fnc_getUnitAnim) select 0) == "stand") then {
//	_caller playMove "AmovPercMstpSrasWrflDnon_diary";
// };
...
if (isNil "_stretcher_object") then {
	_stretcher_object = _stretcher;
};
		...
		switch (toLower _position) do {
			case "front": {
				...
				if (local _caller) then {
					_stretcher_monitoring = _caller getVariable "ace_stretcher_monitoring";
				};
...
if (_stretcher_monitoring) then {
	...
	while {(_caller getVariable "ace_stretcher_monitoring")} do {
		...
		if (_count > 8 && {isNil "_carrier_back"}) then {
			...
			if (local _carrier_front) then {
				"You need someone helping you." call ace_fnc_visual;
			};
		};
		...
		if ((velocity _carrier_front call ACE_fnc_magnitude) > 5.51) then {
			...
			if (local _carrier_front) then {
				"You have to walk slowly" call ace_fnc_visual;
			};
		};
		...
		if (((_carrier_front call CBA_fnc_getUnitAnim) select 0) != "stand") then {
			...
			_carrier_front removeAction GVAR(drop_action);
			if (local _carrier_front) then {
				"Get up stand up ..." call ace_fnc_visual;
			};
		};
		if (((_carrier_back call CBA_fnc_getUnitAnim) select 0) != "stand") then {
			call FUNC(sdrop);
			_carrier_front removeAction GVAR(drop_action);
			if (local _carrier_back) then {
				"Get up stand up ..." call ace_fnc_visual;
			};
		};
		...
		if ((animationState _carrier_front) in ["ladderrifleon","laddercivilon"]) then {
			...
			_carrier_front removeAction GVAR(drop_action);
			if (local _carrier_front) then {
				"No ladders dude, no ladders" call ace_fnc_visual;
			};
		};
