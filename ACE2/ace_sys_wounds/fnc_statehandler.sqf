...
while {true} do {
	...
	_painAdd = _unit getVariable "ace_w_pain_add"; // This is the pain added every frame for the wounding, since we only use pain once (when shot), we use this as base for calculations
	...
	_painCATAdd = if (_CATapplied && {time - _CATtime > CAT_INDUCED_PAIN_DELAY}) then {
		_painAdd * 0.55
	} else {
		0
	};
	...
	_pain = _unit getVariable "ace_w_pain";
	_painAdd = _unit getVariable "ace_w_pain_add"; // TODO: Pain raises too fast
	...
	switch (_state) do {
		case 800: { // bleeding
			...
			if (_oldstate != _state && {_unit getVariable "ace_w_state" == 800}) then {
				_oldstate = _state
			};
		};
					...
					if (_withPMR) then {
						if (isPlayer _unit) then {
							if (vehicle _unit == _unit) then {
								...
								if (_sl >= 0.78) then {
									[position _unit, _sl, _unit] call FUNC(DoSlope)
								};
							};
						};
					};
	...
	if !(_withPMR) then {
		...
		if (ace_wounds_prevtime > 0 && {isPlayer _unit}) then {
			...
			if (isNil QGVAR(no_rpunish)) then {
				...
				_pun = if (typeName GVAR(no_rpunish) == "BOOL") then {
					!GVAR(no_rpunish)
				} else {
					true
				};
	...
	if (GVAR(auto_assist) && {time > _lastRequest}) then {
		...
		switch _state do {
			...
			case 801: {
				if (_unit call FUNC(isUncon)) then {
					_lastRequest = time + DELAY;
					[_unit] spawn FUNC(requestAssist)
				} else {
