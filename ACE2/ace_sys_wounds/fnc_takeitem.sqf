...
if (_item in (magazines _receiver)) exitWith {
	...
	if !(_isMedic && {(GVAR(medical_oasis) || {_item == BND})}) then {
		...
		if (local _receiver) then {
			_receiver removeMagazine _item
		} else {
			[QGVAR(remove_ruckmag), [_receiver, _item]] call ACE_fnc_receiverOnlyEvent
		};
	};
	true;
};
...
if (_item in (magazines _helper)) exitWith {
	...
	if !(_isMedic && {(GVAR(medical_oasis) || {_item == BND})}) then {
		...
		if (local _helper) then {
			_helper removeMagazine _item
		} else { [QGVAR(remove_mag), [_helper, _item]] call ACE_fnc_receiverOnlyEvent
		};
	};
	true;
};
