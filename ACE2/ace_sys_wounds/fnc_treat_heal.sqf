...
if !(alive _unit) exitWith {
	if (_isTheP) then {
		hintSilent (localize "STR_ACE_WOUNDS_HEISDEAD")
	};
};
...
if !(_allowed) exitWith {
	if (_isTheP) then {
		hintSilent "No MedKit"
	};
};

if (_damage == 0) exitWith {
	if (_isTheP) then {
		hintSilent "He looks fine"
	};
};

if (_isTheP) then {
	hintSilent ("Treating: " + name _unit)
};

_nearMedicFacility = [_unit] call FUNC(nearMedicalFacility);
_coef = if (_nearMedicFacility) then {
	0.5
} else {
	1
};
...
if (_unit != _healer) then {
	[QGVAR(surg), [_unit,_timeOut]] call ACE_fnc_receiverOnlyEvent
};
...
if (_healer getVariable "ACE_PB_Result" == 1) then {
	_treated = if (_nearMedicFacility) then {
		localize "STR_ACE_WOUNDS_HEALED"
	} else {
		localize "STR_ACE_WOUNDS_PATCHED"
	};
	if (_isTheP) then {
		hintSilent (localize "STR_ACE_WOUNDS_SUCK" + " " + _treated)
	};
...
if (_unit != _healer) then {
	[QGVAR(surg_abort), _unit] call ACE_fnc_receiverOnlyEvent
};