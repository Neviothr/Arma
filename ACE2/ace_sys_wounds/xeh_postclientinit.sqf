...
[] spawn {
	...
	while {true} do {
		...
		_ar = if (_vehicle != _unit) then {
			[_vehicle]
		} else {
			(nearestObjects [_unit, [], VEHICLE_DISTANCE]) - [_unit]
		};
		{
			_xtra = if (_x isKindOf "CAManBase") then {
				(_x distance _unit) <= INFANTRY_DISTANCE
			} else {
				if (_x isKindOf "AllVehicles") then {
					true
				} else {
					(_x distance _unit) <= OBJECT_DISTANCE
				}
			};
		...
		if (_burn || {_unitBurns}) then {
			...
			if !(_unitBurns) then {
				if (_j > DAM_LIMIT) then {
					[_unit, ITERATION_DAMAGE] call FUNC(addDamage)
				} else {
					INC(_j)
				};
			} else {
			...
			// Set _unit on fire!
			_bc = if (_unit isKindOf "Civilian") then {
				CIV_LIMIT
			} else {
				INF_LIMIT
			};
...
FUNC(DoSlope) = {
	...
	if (_found) then {
		_unit setPos [_pos select 0, _pos select 1, _unit distance _pos]
	};
};
