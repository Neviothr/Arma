...
_coef = if ([player] call FUNC(isMedic)) then {
	0.5
} else {
	1
};
...
if (player != _receiver) then {
	[QGVAR(stabilize), [_receiver, _timeOut]] call ACE_fnc_receiverOnlyEvent
};
...
if (_receiver != player) then {
	[QGVAR(stabilize_abort), _receiver] call ACE_fnc_receiverOnlyEvent
};