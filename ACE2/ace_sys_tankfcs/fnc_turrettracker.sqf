...
[_this,_timestep] spawn { // smoothly interpolate turret speeds for every frame
	...
	while { _computelead } do {
		...
		// Run at least twice
		if (count _tazimuth >= 2) then { _computelead = _veh getVariable "ace_compute_lead" };

		if (count _tazimuth >= 2 && {!(_veh getVariable "ace_compute_lead")}) then {
...
while { _computelead } do { // update turret position every _timestep
	...
	if (_azcount >= 2) then {
		...
		if (_tazichange > 180) then {_tazichange = _tazichange-360};
		if (_tazichange < -180) then {_tazichange = _tazichange+360};
		_tazichangeprev = _this getvariable "ace_tankfcs_azichangeprev";
	...
	// Run at least twice
	if ( count _tazimuth >= 2 ) then { _computelead = _this getVariable "ace_compute_lead" };
