...
while { !_poschanged && {vehicle player == _veh} } do {
	...
	if (_pos select 0 == "Turret") then { //plr in gunner pos, checking if it's the same or another turret
		...
		{
			if (getNumber(configfile>>"cfgvehicles">>(typeof _veh)>>"ace_sys_reticles_gunneroptics")==1) then
			{
				if !(ace_sys_reticles_sightup_veh) then {player spawn ace_sys_reticles_fnc_veh_reticle};
			};
		};
...
_veh removeaction _action_dlg;
if !(ACE_SYS_SA_TEACTIVE) then { _veh removeaction _action_TE };
