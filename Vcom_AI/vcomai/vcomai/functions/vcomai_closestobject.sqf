...
if (TypeName _object isEqualTo "STRING") then {
	_position = getMarkerPos _object;
};
if (TypeName _object isEqualTo "ARRAY") then {
	_position = _object;
};
...

{
	if !(isNil "_x") then {
		...
		if (TypeName _x isEqualTo "OBJECT") then {
			_CompareObjectPos = getPosWorld _x;
		};
		if (TypeName _x isEqualTo "STRING") then {
			_CompareObjectPos = getMarkerPos _x;
		};
		if (TypeName _x isEqualTo "ARRAY") then {
			_CompareObjectPos = _x;
		};
