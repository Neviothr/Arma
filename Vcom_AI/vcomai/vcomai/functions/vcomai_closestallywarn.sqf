...
if !((side _UnitGroup) in VCOM_SideBasedMovement) exitWith {
	if (VCOM_AIDEBUG isEqualTo 1) then {
		systemChat format ["Exited ClosestAllyWarn1..: %1", _UnitGroup];
	};
};
...
if (isPlayer _Unit) exitWith {
	if (VCOM_AIDEBUG isEqualTo 1) then {
		systemChat format ["Exited ClosestAllyWarn1a..: %1", _UnitGroup];
	};
};
...
if (_Unit in ArtilleryArray) then {
	ArtilleryArray = ArtilleryArray - [_Unit];
};
...
if (_NoFlanking || {_GrabVariable} || {!("ItemRadio" in _CheckStatus)}) exitWith {
	if (VCOM_AIDEBUG isEqualTo 1) then {
		systemChat format ["Exited ClosestAllyWarn2..: %1", _UnitGroup];
	};
};
