...
if (_GroupLeader isEqualTo _Unit) then {
	_VCOM_GroupLeader = true;
} else {
	_VCOM_GroupLeader = false;
};

_SubLeader = isFormationLeader _Unit;
if (_SubLeader) then {
	_VCOM_SubLeader = true;
} else {
	_VCOM_SubLeader = false;
};
...
if (_GroupLeader distance _Unit > 120) then {
	_Unit forceSpeed -1;
	_Unit doMove (getPos _GroupLeader);
};
