private ["_Vcom_Indoor", "_cansee", "_position", "_Unit", "_weapon", "_leader", "_gunner", "_assistant", "_UnitGroups", "_CurrentBackPack", "_class", "_parents", "_rnd", "_dist", "_dir", "_positions", "_myNearestEnemy", "_StaticClassName", "_IsMortar", "_WeaponClassname", "_StaticCreated", "_dirTo"];
  	...
  	if (!(isNil "_class")) then {
    	...
    	if (!(isNil "_parents")) then {
      		if (("StaticWeapon" in _parents) || {("Weapon_Bag_Base" in _parents)}) then {
      			_Unit setVariable ["USEDSTATICWEAP", _CurrentBackPack, false];
      			_gunner = _Unit;
      		};
    	};
  	};
