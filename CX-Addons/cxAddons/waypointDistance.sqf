...
addMissionEventHandler ["Draw3D", {
    if (!isNil "TRN_var_customMarkLocation") then {
        drawIcon3D ["\A3\ui_f\data\igui\cfg\cursors\customMark_ca.paa", [1,1,1,0.8], [TRN_var_customMarkLocation select 0, TRN_var_customMarkLocation select 1, (TRN_var_customMarkLocation select 2) + 2], 1.1, 1.3, 0, "", 1, 0, "TahomaB", "", true];
    };
}];
};
